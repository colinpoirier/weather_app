      ...
      child: FadeTransition(
        ...
        child: CustomPaint(
          painter: _DropdownMenuPainter(
            color: Colors.transparent,//blue[900].withOpacity(0.4),//Theme.of(context).canvasColor,
...
class _DropdownMenuRouteLayout<T> extends SingleChildLayoutDelegate {
  ...
  BoxConstraints getConstraintsForChild(BoxConstraints constraints) {
    ...
    final double maxHeight = (5 * _kMenuItemHeight);
    // The width of a menu should be at most the view width. This ensures that
    // the menu does not extend past the left and right edges of the screen.
    final double width = math.min(constraints.maxWidth, buttonRect.width);
    //_kwidth = width.clamp(0.0, 220.0);
    return BoxConstraints(
      minWidth: width,//.clamp(0.0, 220.0),
      maxWidth: width,//.clamp(0.0, 220.0),
      minHeight: 0.0,
      maxHeight: maxHeight,
    );
  }
...
class _DropdownRoute<T> extends PopupRoute<_DropdownRouteResult<T>> {
  ...
  Widget buildPage(BuildContext context, Animation<double> animation, Animation<double> secondaryAnimation) {
    ...
    double menuTop = (buttonTop) - (_kMenuItemHeight - buttonRect.height) / 2.0;
...
class MyDropdownMenuItem<T> extends StatelessWidget {
...
class MyDropdownButtonHideUnderline extends InheritedWidget {
  /// Creates a [DropdownButtonHideUnderline]. A non-null [child] must
  /// be given.
  const MyDropdownButtonHideUnderline({
    Key key,
  ...
  static bool at(BuildContext context) {
    return context.inheritFromWidgetOfExactType(MyDropdownButtonHideUnderline) != null;
  }

  @override
  bool updateShouldNotify(MyDropdownButtonHideUnderline oldWidget) => false;
}
...
class MyDropdownButton<T> extends StatefulWidget {
  ...
  }) : assert(items == null || value == null || items.where((MyDropdownMenuItem<T> item) => item.value == value).length == 1),
      super(key: key);
...
class _DropdownButtonState<T> extends State<MyDropdownButton<T>> with WidgetsBindingObserver {
  ...
  void _updateSelectedIndex() {
    ...
    assert(widget.value == null ||
      widget.items.where((MyDropdownMenuItem<T> item) => item.value == widget.value).length == 1);
    _selectedIndex = null;
